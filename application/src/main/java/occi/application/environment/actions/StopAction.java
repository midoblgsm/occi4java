 ...
 * In this case the StopAction creates a new definition for an Environment resource.
 * 
 * @see Action
 * @author Sami Yangui
 */
public class StopAction extends Action {
	...
	public void execute(URI uri, Method method) {
		LOGGER.debug("libvirt: Stop Environment");
		...
		for (UUID uuid : Environment.getEnvironmentList().keySet()) {
			if (uuid.equals(environmentUuid)) {
				environment = Environment.getEnvironmentList().get(environmentUuid);
			}
		}
		EnvironmentInterface environmentInterface=new EnvironmentInterfaceImpl();
		environmentInterface.stopEnvironment(environment);
