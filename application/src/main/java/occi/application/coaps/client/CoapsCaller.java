 ...
 */
package occi.application.coaps.client;
...
public class CoapsCaller {
	 ...
	 * COAPS API location
	 */
	private final static String apiLocation = "http://localhost:8080/CF-api/rest";
	...
	public static String getAppURL(ClientResponse cr) {
		return cr.getEntity(ApplicationType.class).getUris().getUri().get(0);
	}

	public static String getAppID(ClientResponse cr) {
		return cr.getEntity(ApplicationType.class).getAppId().toString();
	}

	public static String getEnvID(ClientResponse cr) {
		return cr.getEntity(EnvironmentType.class).getEnvId().toString();
	}
