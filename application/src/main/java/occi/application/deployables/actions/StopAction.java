 ...
 * In this case the StopAction creates a new definition for an Deployables resource.
 * 
 * @see Action
 * @author Sami Yangui
 */
public class StopAction extends Action {
	...
	public void execute(URI uri, Method method) {
		LOGGER.debug("libvirt: Stop Deployables");
		...
		for (UUID uuid : Deployable.getDeployablesList().keySet()) {
			if (uuid.equals(deployablesUuid)) {
				deployables = Deployable.getDeployablesList().get(deployablesUuid);
			}
		}
		DeployablesInterface deployablesInterface=new DeployablesInterfaceImpl();
		deployablesInterface.stopDeployables(deployables);
