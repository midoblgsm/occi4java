 ...
 */
public class OcciRestReconfigurationManagers extends ServerResource {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(OcciRestReconfigurationManagers.class);

	/**
	 * Returns all ReconfigurationManager resources.
	 * 
	 * @return ReconfigurationManager resources String
	 */
	@Get
	public String getOCCIRequest(Representation representation) {
		...
		Map<UUID, ReconfigurationManager> reconfigurationManagerList = ReconfigurationManager.getReconfigurationManagerList();
		// initialize buffer
		...
		LOGGER.debug("Size of ReconfigurationManager list: " + reconfigurationManagerList.size());
		// iterate through all available container resources
		int i = 1;
		for (UUID id : reconfigurationManagerList.keySet()) {
			reconfigurationManager = reconfigurationManagerList.get(id);
			buffer.append(getReference());
			buffer.append(reconfigurationManager.getId());
			if (i < reconfigurationManagerList.size()) {
				buffer.append(",");
			}
			i++;
		}
		...
		if (reconfigurationManagerList.size() <= 0) {
			// return http status code
			getResponse().setStatus(Status.SUCCESS_NO_CONTENT,
					buffer.toString());
			return "There are no ReconfigurationManager resources";
