...
import telecom.sudparis.occi.autonomic.SubscriptionLink.State;
...
public class OcciRestSubscriptionLink extends ServerResource {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(OcciRestSubscriptionLink.class);
	...
	public String postOCCIRequest(Representation representation)
		...
		if (getReference().toString().contains("subscribe")) {
			String subscriptionString = representation.getText();
			Subscription subscription = XMLTools
					.getSubscriptionFromXML(subscriptionString);
			SubscriptionLink subscriptionLink = SubscriptionLink
					.getSubscriptionLinkList().get(
							UUID.fromString(getReference().getLastSegment()));
			subscriptionLink.subscribe(subscription);
			...
			try {
				...
				if (!getReference().toString().contains("action")) {
					...
					buffer.append("occi.subscriptionlink.name=").append(
							xoccimap.get("occi.subscriptionlink.name"));

					buffer.append(" occi.subscriptionlink.version=").append(
							xoccimap.get("occi.subscriptionlink.version"));
					buffer.append(" occi.subscriptionlink.state=").append(
							"available");
					...
					Mixin<SubscriptionLink> subscriptionLinkWithMixin = new Mixin(
							SubscriptionToolMixinInterface.class,
							SubscriptionToolMixinImpl.class,
							SubscriptionLink.class);
					Factory factory = dynamicMixer.getFactory(
							SubscriptionLink.class, subscriptionLinkWithMixin);
					...
					Object[] objects0 = {
							source,
							target,
							(String) xoccimap.get("occi.subscriptionlink.name"),
							(String) xoccimap
									.get("occi.subscriptionlink.version"),
							State.available, set };
					SubscriptionLink actionLink = (SubscriptionLink) factory
							.newInstance(clazz0, objects0);

					actionLink.setKind(new Kind(null, "subscriptionlink",
					...
					if (xoccimap.get("occi.subscriptionlink.Category") != null)
						if (!"subscriptionlink".equalsIgnoreCase(xoccimap.get(
								"occi.subscriptionlink.Category").toString())) {
							throw new IllegalArgumentException(
									"Illegal Category type: "
											+ xoccimap
													.get("occi.subscriptionlink.Category"));
						}
					...
					LOGGER.debug("subscriptionlink Uuid: "
							+ actionLink.getUuid());
					LOGGER.debug("subscriptionlink Kind scheme: "
					...
					setLocationRef((getRootRef().toString() + actionLink
							.getId()));
					...
					SubscriptionLink subscriptionLink = SubscriptionLink
							.getSubscriptionLinkList().get(id);
	...
	public String getOCCIRequest() {
		...
		try {
			...
			buffer.append(" Category: ")
					.append(subscriptionLink.getKind().getTerm())
					.append(" scheme= ")
					.append(subscriptionLink.getKind().getScheme())
					.append(" class=\"kind\";").append(" Name: ")
					.append(subscriptionLink.getName()).append(" Version: ")
					.append(subscriptionLink.getState());
			...
			if (requestHeaders.getFirstValue(acceptCase).equals("text/occi")) {
				// generate header rendering
				this.occiCheck.setHeaderRendering(null, subscriptionLink,
	...
	public String deleteOCCIRequest() {
		...
		try {
			...
			if (SubscriptionLink.getSubscriptionLinkList().remove(
					UUID.fromString(subscriptionLink.getId().toString())) == null) {
		...
		try {
			...
			if (xocciattributes != null) {
				...
				if (!xoccimap.isEmpty()) {
					...
					if (xoccimap.containsKey("occi.subscriptionlink.version")) {
						LOGGER.info((String) xoccimap
								.get("occi.subscriptionlink.version"));
						subscriptionLink.setVersion(xoccimap.get(
								"occi.subscriptionlink.version").toString());
					}
					if (xoccimap.containsKey("occi.subscriptionlink.name")) {
						LOGGER.info((String) xoccimap
								.get("occi.subscriptionlink.name"));
						subscriptionLink.setName((String) xoccimap
								.get("occi.subscriptionlink.name"));
					}
