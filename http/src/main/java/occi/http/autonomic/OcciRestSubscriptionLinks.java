 ...
 */
public class OcciRestSubscriptionLinks extends ServerResource {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(OcciRestSubscriptionLinks.class);

	/**
	 * Returns all SubscriptionLink resources.
	 * 
	 * @return SubscriptionLink list String
	 */
	@Get
	public String getOCCIRequest(Representation representation) {
		...
		Map<UUID, SubscriptionLink> subscriptionLinkList = SubscriptionLink.getSubscriptionLinkList();
		// initialize buffer
		...
		LOGGER.debug("Size of SubscriptionLink list: " + subscriptionLinkList.size());
		// iterate through all available container resources
		int i = 1;
		for (UUID id : subscriptionLinkList.keySet()) {
			subscriptionLink = subscriptionLinkList.get(id);
			buffer.append(getReference());
			buffer.append(subscriptionLink.getId());
			if (i < subscriptionLinkList.size()) {
				buffer.append(",");
			}
			i++;
		}
		...
		if (subscriptionLinkList.size() <= 0) {
			// return http status code
			getResponse().setStatus(Status.SUCCESS_NO_CONTENT,
					buffer.toString());
			return "There are no SubscriptionLink resources";
