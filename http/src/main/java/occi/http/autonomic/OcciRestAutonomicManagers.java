 ...
 */
public class OcciRestAutonomicManagers extends ServerResource {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(OcciRestAutonomicManagers.class);

	/**
	 * Returns all AutonomicManager resources.
	 * 
	 * @return AutonomicManager list String
	 */
	@Get
	public String getOCCIRequest(Representation representation) {
		...
		Map<UUID, AutonomicManager> autonomicManagerList = AutonomicManager.getAutonomicManagerList();
		// initialize buffer
		...
		LOGGER.debug("Size of AutonomicManager list: " + autonomicManagerList.size());
		// iterate through all available container resources
		int i = 1;
		for (UUID id : autonomicManagerList.keySet()) {
			autonomicManager = autonomicManagerList.get(id);
			buffer.append(getReference());
			buffer.append(autonomicManager.getId());
			if (i < autonomicManagerList.size()) {
				buffer.append(",");
			}
			i++;
		}
		...
		if (autonomicManagerList.size() <= 0) {
			// return http status code
			getResponse().setStatus(Status.SUCCESS_NO_CONTENT,
					buffer.toString());
			return "There are no AutonomicManager resources";
