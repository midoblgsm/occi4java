 ...
 */
public class OcciRestNotificationLinks extends ServerResource {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(OcciRestNotificationLinks.class);
	...
	public String getOCCIRequest(Representation representation) {
		...
		Map<UUID, NotificationLink> notificationLinkList = NotificationLink.getNotificationLinkList();
		// initialize buffer
		...
		LOGGER.debug("Size of NotificationLink list: " + notificationLinkList.size());
		// iterate through all available container resources
		int i = 1;
		for (UUID id : notificationLinkList.keySet()) {
			notificationLink = notificationLinkList.get(id);
			buffer.append(getReference());
			buffer.append(notificationLink.getId());
			if (i < notificationLinkList.size()) {
				buffer.append(",");
			}
			i++;
		}
		...
		if (notificationLinkList.size() <= 0) {
			// return http status code
			getResponse().setStatus(Status.SUCCESS_NO_CONTENT,
					buffer.toString());
			return "There are no NotificationLink resources";
