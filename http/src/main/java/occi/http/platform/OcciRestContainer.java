...
import occi.platform.Container.Architecture;
import occi.platform.Container;
import occi.platform.Container.State;
import occi.platform.container.actions.CreateAction;
import occi.platform.container.actions.DeleteAction;
...
public class OcciRestContainer extends ServerResource {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(OcciRestContainer.class);
	...
	public String postOCCIRequest(Representation representation)
		...
		try {
			...
			if (!getReference().toString().contains("action")) {
				...
				buffer.append("occi.container.name=").append(
						xoccimap.get("occi.container.name"));
				buffer.append(" occi.container.version=").append(
						xoccimap.get("occi.container.version"));
				buffer.append(" occi.container.architecture=").append(
						xoccimap.get("occi.container.architecture"));
				buffer.append(" occi.container.state=").append("unavailable");
				...
				Container container = new Container(
						(String) xoccimap.get("occi.container.name"),
						(String) xoccimap.get("occi.container.version"),
						Architecture.valueOf((String) xoccimap.get("occi.container.architecture")),
						State.unvailable,
						set);

				URI uri = new URI(container.getId().toString());
				...
				for (Mixin mixin : Mixin.getMixins()) {
					if (mixin.getEntities() != null) {
						if (mixin.getEntities().contains(container)) {
				...
				LOGGER.debug("Container Uuid: " + container.getUuid());
				LOGGER.debug("Container Kind scheme: "
						+ container.getKind().getScheme());
				// Check accept header
				if (requestHeaders.getFirstValue(acceptCase)
					...
					this.occiCheck.setHeaderRendering(null, container,
							buffer.toString(), null);
					getResponse().setEntity(representation);
					getResponse().setStatus(Status.SUCCESS_OK);
				}
				// Location Rendering in HTTP Header, not in body
				setLocationRef((getRootRef().toString() + container.getId()));
				...
				if (actionName.length >= 2) {
					...
					if (actionName[1].equalsIgnoreCase("start")) {
						...
						container.getStart().execute(URI.create(location),null);
						// Set the current state of the container resource
						container.setState(State.available);
					}

					if (actionName[1].equalsIgnoreCase("stop")) {
						LOGGER.debug("Stop Action called.");
						// Call the Stop action of the container resource
						container.getStop().execute(URI.create(location),null);
						// Set the current state of the container resource
						container.setState(State.unvailable);
					}
					...
					if (actionName[1].equalsIgnoreCase("restart")) {
						...
						container.getRestart().execute(URI.create(location), null);
						// Set the current state of the container resource
						container.setState(State.available);
					}
				}
	...
	public String getOCCIRequest() {
		...
		try {
			...
			Container container = Container.getContainerList().get(UUID
					.fromString(getReference().getLastSegment()));

			// put all attributes into a buffer for the response
			StringBuffer buffer = new StringBuffer();
			StringBuffer linkBuffer = new StringBuffer();
			buffer.append(" Category: ").append(container.getKind().getTerm())
					.append(" scheme= ").append(container.getKind().getScheme())
					.append(" class=\"kind\";").append(" Name: ")
					.append(container.getName()).append(" Version: ")
					.append(container.getVersion()).append(" Architecture: ")
					.append(container.getArchitecture()).append(" State: ")
	...
	public String deleteOCCIRequest() {
		...
		try {
			...
			deleteAction.execute(new URI(container.getId().toString()), null);
			// remove it from container resource list
			if (Container.getContainerList().remove(UUID.fromString(container.getId()
					.toString())) == null) {
	...
	public String putOCCIRequest(Representation representation)
		...
		try {
			...
			if (xocciattributes != null) {
				...
				if (!xoccimap.isEmpty()) {
					// Change the container attribute if it is send by the request

					if (xoccimap.containsKey("occi.container.architecture")) {
						LOGGER.info((String) xoccimap
								.get("occi.container.architecture"));
						container.setArchitecture(Architecture
								.valueOf((String) xoccimap
										.get("occi.container.architecture")));
					}
					if (xoccimap.containsKey("occi.container.version")) {
						LOGGER.info((String) xoccimap.get("occi.container.version"));
						container.setVersion(xoccimap.get(
								"occi.container.version").toString());
					}
					if (xoccimap.containsKey("occi.container.name")) {
						LOGGER.info((String) xoccimap
								.get("occi.container.name"));
						container.setName((String) xoccimap
								.get("occi.container.name"));
					}
