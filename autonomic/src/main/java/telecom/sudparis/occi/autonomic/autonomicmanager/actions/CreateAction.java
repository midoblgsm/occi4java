 ...
 * a new definition for an Analyzer resource.
 * 
 */
public class CreateAction extends Action {
	...
	public void execute(URI uri, Method method) {
		LOGGER.debug("libvirt: Create Container");
		...
		for (UUID uuid : AutonomicManager.getAutonomicManagerList().keySet()) {
			if (uuid.equals(autonomicManagerUuid)) {
				autonomicManager = AutonomicManager.getAutonomicManagerList()
						.get(autonomicManagerUuid);
			}
		}
		AutonomicManagerInterface autonomicManagerInterface = new AutonomicManagerInterfaceImpl();
		autonomicManagerInterface.createAutonomicManager(autonomicManager);
